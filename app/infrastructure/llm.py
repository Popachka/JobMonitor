...
def get_vacancy_parse_agent() -> Agent[None, OutVacancyParse]:
    ...
    system_prompt = (
        ...
        "4. 'tech_stack': извлекай конкретные технологии (FastAPI, PostgreSQL и т.д.).\n"
        "5. 'salary': если указан диапазон (например, 15000-20000) — бери минимальную сумму. "
        "Если зарплата не указана — верни null. Валюту бери из описания, если не указана — null.\n"
        "6. 'work_format': выбирай одно из [REMOTE, HYBRID, ONSITE, UNDEFINED]. "
...
def get_resume_parse_agent() -> Agent[None, OutResumeParse]:
    ...
    system_prompt = (
        ...
        "   - Если сопоставить нельзя, не добавляй язык.\n"
        "   - Не определяй язык только по инфраструктуре (Docker, Kubernetes, PostgreSQL и т.п.).\n"
        "4. 'tech_stack': извлеки фреймворки и БД (FastAPI, PostgreSQL). "
        "Не дублируй языки.\n"
        "5. 'salary': желаемая зарплата. Если указан диапазон — бери минимум. "
        ...
        "6. 'work_format': выбирай одно из [REMOTE, HYBRID, ONSITE, UNDEFINED]. "
        "Если нет явного указания — UNDEFINED.\n"
        "7. 'full_relevant_text_from_resume': сохрани текст без изменений.\n"
    )
